...
+ (void)setPadButton:(int)pad button:(int)button pressed:(BOOL)pressed {
    // Forward to PAD system (Audit Sec 2.7)
    if (button <= PAD_R2) {
        Pad::SetButtonState(pad, button, pressed);
    }
}

+ (void)setAnalogStick:(int)pad stick:(int)stick x:(float)x y:(float)y {
    Pad::SetAxisState(pad, stick == 0 ? Pad::Axis_LeftX : Pad::Axis_RightX, x);
    Pad::SetAxisState(pad, stick == 0 ? Pad::Axis_LeftY : Pad::Axis_RightY, y);
}

+ (void)clearPadState:(int)pad {
    for (int i = 0; i < 12; i++)
        Pad::SetButtonState(pad, i, false);
}
