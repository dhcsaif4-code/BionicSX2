...
// STATUS: YELLOW
// RETAINED: Mach exception handler for SIGSEGV → vtlb page fault handling
// RETAINED: sysctl CPU class detection (works on iOS ARM64)
// REMOVED: CoreGraphics dependency (not available on iOS)
// REMOVED: IOKit power management calls
...
static std::optional<T> sysctlbyname_T(const char* name)
{
	...
	{
		ERROR_LOG("(PageFaultHandler) sysctl {} gave unexpected size {}", name, output_size);
		return std::nullopt;
	}
	return output;
}
...
// --- PageFaultHandler (Mach exception ports) ---
// RETAINED: Mach exception ports work identically on iOS (Audit Sec 6.2)
